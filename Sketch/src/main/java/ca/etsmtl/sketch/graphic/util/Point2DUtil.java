...
public class Point2DUtil {
    ...
    static public boolean isPointInsidePolygon(ArrayList<Point2D> polygonPoints, Point2D q) {
        ...
        // where it (the code, not the algorithm) is attributed to Randolph
        // Franklin.
        ...
        for (i = 0, j = polygonPoints.size() - 1; i < polygonPoints.size(); j = i++) {
            ...
            if ((((yi <= q.y()) && (q.y() < yj)) || ((yj <= q.y()) && (q.y() < yi)))
    ...
    // Uses the well known algorithm "Graham's scan" for computing the convex
    // hull in 2D,
    ...
    static public ArrayList<Point2D> computeConvexHull(
        ...
        // Of these, we find the one with minimal x coordinate (the
        // "bottom left" point)
        ...
        // Imagine that for each point, we compute the point's angle with
        // respect to bottomLeftPoint,
        // and then sort the points by this angle.
        // This is equivalent to sorting the points by their cotangent, which is
        // faster to compute.
        ...
        for (int i = 0; i < points.size(); ++i) {
            ...
            if (delta_y == 0) {
                ...
                pointsWithCotangents[i] = new Point2DAndScore(p, delta_x / delta_y /*
                                                                                     * the
																					 * cotangent
																					 */, false);
            }
        ...
        // We'll need to be able to efficiently remove points from
        // consideration,
        // so we copy them into a linked list.
        // In doing this, we also reverse the order of points
        // (so they are in descending order of cotangent, i.e., in
        ...
        if (orderedPoints.size() > 2) {
            // We will loop through the ordered points, processing 3 consecutive
            // points at a time.
            ...
            while (true) {
                ...
                // (Note that the x and y components of the cross product are
                // zero,
                ...
                if (crossProduct_z > 0) {
                    ...
                    // or the points are collinear (with the 3rd point either in
                    // front, or behind, the 2nd)
                    // In any case, we remove the 2nd point from consideration
                    // and (try to) backup.
                    ...
                    assert it3.hasNext();
                    it3.next(); // now the iterator is back to where it used to
                    // be
    ...
    static public ArrayList<Point2D> computeExpandedPolygon(ArrayList<Point2D> points, // input
        ...
        if (points.size() == 0) {
            ...
            for (int i = 0; i < points.size(); ++i) {
                ...
                newPoints.add(Point2D.sum(p,
                        Vector2D.mult(new Vector2D(v_previous.y(), -v_previous.x()), marginThickness)));
                newPoints.add(Point2D.sum(p,
                        Vector2D.mult(Vector2D.sum(v_next, v_previous).normalized(), marginThickness)));
    ...
    static public boolean principleComponentAnalysis(
            ...
            Vector2D eigenvector1, Vector2D eigenvector2, double[] eigenvalues // output;
            // caller
            // must
            // pass
            // in
        ...
        // then covXX*covYY is nearly equal to covXY^2, but that's not a
        // problem.
    ...
    // http://doi.acm.org/10.1145/502348.502371
    //
    static public void transformPointsBasedOnDisplacementOfOnePoint(ArrayList<Point2D> points,
                                                                    // these should, of course, be in the same coordinate system as the points
                                                                    // to transform
        ...
        Point2D newCentroid = Point2D.sum(P_new, Vector2D.mult(v2.normalized(), -lengthToPreserve));
    ...
    static public void compute2DTransformBasedOnDisplacementOfTwoPoints(
            // input
            Point2D A_old, Point2D B_old, Point2D A_new, Point2D B_new,

            // output
            Vector2D translation, Point2D rotationCenter,
            // Output. Caller must pass in a 1-element array used to pass value
            // back to caller.
            // Angle is in radians.
            // A positive value means a counterclockwise rotation in a (x+
            // right, y+ up) system.
            float[] rotationAngle,
            // Output. Caller must pass in a 1-element array used to pass value
            // back to caller.
    ...
    // This can be used to implement bimanual (2-handed) manipulation,
    // or 2-finger manipulation, as in a "pinch" gesture
    static public void transformPointsBasedOnDisplacementOfTwoPoints(ArrayList<Point2D> points,
                                                                     // these should, of course, be in the same coordinate system as the points
                                                                     // to transform
                                                                     Point2D A_old, Point2D B_old, Point2D A_new, Point2D B_new) {
        ...
        compute2DTransformBasedOnDisplacementOfTwoPoints(A_old, B_old, A_new, B_new, translation, rotationCenter,
    ...
    //
    static public float computeAverageAngle(ArrayList<Float> angles, // angles,
                                            // in
                                            // radians
                                            float defaultAverageAngle // average angle to return if the given
                                            // angles all cancel out, or if
                                            // angles.size() == 0
    ) {
        Vector2D averageRotationVector = new Vector2D();
        for (float angle : angles) {
            averageRotationVector = Vector2D.sum(averageRotationVector, new Vector2D((float) Math.cos(angle),
    ...
    static public float computeAverageAngle(ArrayList<Float> angles) {
