package ca.etsmtl.sketch.graphic.shape;
...
public class AlignedRectangle2D {
    ...
    public boolean contains(Point2D p) {
        return !isEmpty && min.x() <= p.x() && p.x() <= max.x() && min.y() <= p.y() && p.y() <= max.y();
    }
    ...
    public boolean contains(AlignedRectangle2D r) {
        ...
        if (r.isEmpty)
            return true;
        return min.x() <= r.min.x() && r.max.x() <= max.x() && min.y() <= r.min.y() && r.max.y() <= max.y();
    }
